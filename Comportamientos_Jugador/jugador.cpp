...
//coste
int getCoste(int costeAnt,const Action &a,bool bikini, bool zapatillas, const ubicacion &j, const vector<vector<unsigned char>> &mapa){
	int coste = costeAnt;
	char terreno = mapa[j.f][j.c];
...
//¿item en siguiente ubicacion?
stateN2 item(const stateN2 &st, const ubicacion &j, const vector<vector<unsigned char>> &mapa){
	stateN2 sig_estado = st;
	char terreno = mapa[j.f][j.c];
	switch(terreno){
		case 'K':
			sig_estado.bikini = true;
			break;
		case 'D':
			sig_estado.zapas = true;
	}
	return sig_estado;
}
...
stateN2 applyN2(const Action &a, const stateN2 &st, const vector<vector<unsigned char>> mapa){
	...
	case actWALK: // Si la proxima casilla es transitable y NO está ocupada por el colaborador
		...
		if (CasillaTransitable(sig_ubicacion, mapa) && !(sig_ubicacion.f == st.colaborador.f && sig_ubicacion.c == st.colaborador.c)){
			st_result.jugador = sig_ubicacion;
			st_result.coste = getCoste(st_result.coste,actWALK,st_result.bikini,st_result.zapas,st_result.jugador,mapa);
	...
	case actRUN: // Si las proximas 2 casillas son transitables y NO está ocupada por el colaborador
		...
		if (CasillaTransitable(sig_ubicacion, mapa) && !(sig_ubicacion.f == st.colaborador.f && sig_ubicacion.c == st.colaborador.c)){
			...
			if (CasillaTransitable(sig_ubicacion2, mapa) && !(sig_ubicacion2.f == st.colaborador.f && sig_ubicacion2.c == st.colaborador.c)){
				st_result.jugador = sig_ubicacion2;
				st_result.coste = getCoste(st_result.coste,actRUN,st_result.bikini,st_result.zapas,st_result.jugador,mapa);
				st_result = item(st_result,st_result.jugador,mapa);
			}
		}
		break;
	case actTURN_L:
		st_result.jugador.brujula = static_cast<Orientacion>((st_result.jugador.brujula + 6) % 8);
		st_result.coste = getCoste(st_result.coste,actTURN_L,st_result.bikini,st_result.zapas,st_result.jugador,mapa);
		st_result = item(st_result,st_result.jugador,mapa);
		break;
	case actTURN_SR:
		st_result.jugador.brujula = static_cast<Orientacion>((st_result.jugador.brujula + 1) % 8);
		st_result.coste = getCoste(st_result.coste,actTURN_SR,st_result.bikini,st_result.zapas,st_result.jugador,mapa);
		st_result = item(st_result,st_result.jugador,mapa);
		break;
	}
	return st_result;
